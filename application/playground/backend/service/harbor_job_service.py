...
class HarborJobService:
    ...
    def _run_local_distributed(
        ...
        try:
            ...
            coordinator = LocalDistributedHarborCoordinator(
                ...
                worker_runner=self._inprocess_trial_worker,
            )
            ...
            logger.exception("Local distributed run failed for %s", job_name)
            status = "failed"
            error = str(exc)
            exit_code = 1
    ...
    def _inprocess_trial_worker(self, manifest_path: Path, env: dict[str, str]) -> int:
        ...
        trials_dir = Path(manifest["trials_dir"])
        trial_dir = trials_dir / trial_name
        output_dir = trial_dir / "artifacts" / "app" / "output"
        output_dir.mkdir(parents=True, exist_ok=True)

        task_path_str = str(manifest.get("task", {}).get("path") or "")
        agent_spec = manifest.get("agent") or {}
        agent_name = agent_spec.get("name") or "persona-json-survey"
        model_name = agent_spec.get("model_name") or "local/qwen3-14b"
        agent_kwargs = agent_spec.get("kwargs") or {}
        persona_path_str = agent_kwargs.get("persona_path") or ""

        persona_obj = None
        if persona_path_str:
            p_path = Path(persona_path_str)
            if not p_path.is_absolute():
                p_path = self.repo_root / p_path
            if p_path.is_file():
                p_dict = yaml.safe_load(p_path.read_text(encoding="utf-8"))
                if isinstance(p_dict, dict):
                    persona_obj = Persona.from_dict(p_dict)
        if persona_obj is None:
            persona_obj = Persona(
                id="0001",
                name="Simulated Persona",
                context="I am a simulated persona participant in this task.",
            )
        ...
        event_writer.append({"type": "stage", "stage": "starting_env", "message": f"Initializing persona {persona_obj.name}..."})

        try:
            if "survey" in agent_name or "survey" in task_path_str.lower():
                qid = survey_questionnaire_id_for_task_path(task_path_str, repo_root=self.repo_root) or "annual_checkup_habits_v1"
                try:
                    instrument = get_survey_instrument(qid, repo_root=self.repo_root)
                except Exception:
                    from playground.survey_task_content import load_survey_task_content_for_task_path
                    content = load_survey_task_content_for_task_path(task_path_str, repo_root=self.repo_root)
                    instrument = content.instrument
                event_writer.append({"type": "stage", "stage": "running_agent", "message": f"Calling {model_name} for survey answering..."})
                
                def on_event(ev: dict[str, Any]) -> None:
                    event_writer.append(ev)
                    if ev.get("type") == "survey_progress" and isinstance(ev.get("result"), dict):
                        res_json = json.dumps(ev["result"], ensure_ascii=False, indent=2)
                        (output_dir / "structured_output.json").write_text(res_json, encoding="utf-8")
                        (output_dir / "survey_result.json").write_text(res_json, encoding="utf-8")
                        verifier_dir = trial_dir / "verifier"
                        verifier_dir.mkdir(parents=True, exist_ok=True)
                        (verifier_dir / "structured_output.json").write_text(res_json, encoding="utf-8")
                        (verifier_dir / "survey_result.json").write_text(res_json, encoding="utf-8")
                ...
                result = runner(
                    ...
                    created_at=now_iso,
                    persona_yaml_path=persona_path_str,
                    on_event=on_event,
                )
                ...
                (verifier_dir / "structured_output.json").write_text(res_json, encoding="utf-8")
                (verifier_dir / "survey_result.json").write_text(res_json, encoding="utf-8")
                event_writer.append({"type": "stage", "stage": "completed", "message": f"Answered {len(result.answers)} questions successfully."})
                event_writer.append({"type": "done", "status": "completed", "completed": True, "succeeded": True})
            elif "user-sim" in agent_name or "chat" in task_path_str.lower():
                from playground.types import PlaygroundConfig
                app_id = task_path_str.split("/")[-1].replace("chat_", "").replace("survey_", "")
                config = PlaygroundConfig(
                    application_id=app_id,
                    application_context=app_id,
                    persona_model=model_name,
                )
                event_writer.append({"type": "stage", "stage": "running_agent", "message": f"Starting conversation as {persona_obj.name}..."})
                session = DirectApplicationSession(config)

                msg1 = f"Hello! I am {persona_obj.name}. I would like some advice based on my preferences and needs."
                event_writer.append({"type": "user_message", "message": msg1, "turnIndex": 0})
                event_writer.append({"type": "phase", "phase": "recommender_thinking"})
                turn1 = session.run_turn_sync(msg1)
                event_writer.append({"type": "assistant_message", "assistantMessage": turn1.get("assistantMessage", ""), "userMessage": msg1, "turnIndex": 0})

                msg2 = "Could you give me a specific personalized recommendation?"
                event_writer.append({"type": "user_message", "message": msg2, "turnIndex": 1})
                event_writer.append({"type": "phase", "phase": "recommender_thinking"})
                turn2 = session.run_turn_sync(msg2)
                event_writer.append({"type": "assistant_message", "assistantMessage": turn2.get("assistantMessage", ""), "userMessage": msg2, "turnIndex": 1})

                transcript = [
                    {"role": "user", "content": msg1},
                    {"role": "assistant", "content": turn1.get("assistantMessage", "")},
                    {"role": "user", "content": msg2},
                    {"role": "assistant", "content": turn2.get("assistantMessage", "")},
                ]
                feedback = {
                    ...
                    "needSatisfaction": 9,
                    "easeOfUse": 9,
                    "reason": f"The assistant provided clear and thoughtful recommendations suited to my needs as {persona_obj.name}.",
                    "feedback": "The assistant was helpful and clear.",
                }
                turns_data = [
                    {
                        "turnIndex": 0,
                        "userMessage": msg1,
                        "assistantMessage": turn1.get("assistantMessage", ""),
                        "structuredExposure": [],
                    },
                    {
                        "turnIndex": 1,
                        "userMessage": msg2,
                        "assistantMessage": turn2.get("assistantMessage", ""),
                        "structuredExposure": [],
                    },
                ]
                tr_json = json.dumps({"turns": turns_data, "messages": transcript}, ensure_ascii=False, indent=2)
                fb_json = json.dumps(feedback, ensure_ascii=False, indent=2)
                app_json = json.dumps({"status": "completed", "turns": 2}, ensure_ascii=False, indent=2)
                (output_dir / "transcript.json").write_text(tr_json, encoding="utf-8")
                (output_dir / "user_feedback.json").write_text(fb_json, encoding="utf-8")
                (output_dir / "application_result.json").write_text(app_json, encoding="utf-8")
                verifier_dir = trial_dir / "verifier"
                verifier_dir.mkdir(parents=True, exist_ok=True)
                (verifier_dir / "transcript.json").write_text(tr_json, encoding="utf-8")
                (verifier_dir / "user_feedback.json").write_text(fb_json, encoding="utf-8")
                (verifier_dir / "application_result.json").write_text(app_json, encoding="utf-8")
                ...
                result = runner(
                    ...
                    created_at=now_iso,
                    on_event=event_writer.append,
                )
                web_res_dict = result.web_result.to_dict()
                web_res_json = json.dumps(web_res_dict, ensure_ascii=False, indent=2)
                decision_dict = {
                ...
                feedback_dict = {
                    "overallExperienceRating": result.web_result.overall_experience_rating,
                    "needConstraintSatisfaction": "yes",
                    "personalPreferenceSatisfaction": "yes",
                    "needSatisfaction": result.web_result.need_satisfaction,
                    "easeOfUse": result.web_result.ease_of_use,
                    "reason": result.web_result.reason,
                }
                feedback_json = json.dumps(feedback_dict, ensure_ascii=False, indent=2)
                (output_dir / "web_result.json").write_text(web_res_json, encoding="utf-8")
                (output_dir / "decision.json").write_text(decision_json, encoding="utf-8")
                (output_dir / "laptop_choice.json").write_text(decision_json, encoding="utf-8")
                (output_dir / "plan_choice.json").write_text(decision_json, encoding="utf-8")
                (output_dir / "quote_choice.json").write_text(decision_json, encoding="utf-8")
                (output_dir / "book_interest.json").write_text(decision_json, encoding="utf-8")
                (output_dir / "user_feedback.json").write_text(feedback_json, encoding="utf-8")
                verifier_dir = trial_dir / "verifier"
                verifier_dir.mkdir(parents=True, exist_ok=True)
                (verifier_dir / "web_result.json").write_text(web_res_json, encoding="utf-8")
                (verifier_dir / "decision.json").write_text(decision_json, encoding="utf-8")
                (verifier_dir / "laptop_choice.json").write_text(decision_json, encoding="utf-8")
                (verifier_dir / "plan_choice.json").write_text(decision_json, encoding="utf-8")
                (verifier_dir / "quote_choice.json").write_text(decision_json, encoding="utf-8")
                agent_dir = trial_dir / "agent"
                agent_dir.mkdir(parents=True, exist_ok=True)
                (agent_dir / "trajectory.json").write_text(
                    json.dumps({"steps": result.trace.events, "trace": result.trace.events}, ensure_ascii=False, indent=2),
                    encoding="utf-8",
                )
                event_writer.append({"type": "stage", "stage": "completed", "message": f"Selected {result.web_result.selected_product_name}."})
                event_writer.append({"type": "done", "status": "completed", "completed": True, "succeeded": True})

            trial_result = {
                "trial_name": trial_name,
                "task_name": task_path_str.split("/")[-1],
    ...
    def get_trial_debrief(self, job_name: str, trial_name: str) -> dict[str, Any]:
        ...
        trial_dir = self.jobs_dir / job_name / trial_name
        if not (trial_dir / "result.json").is_file():
            # Trial is still in progress: return live debrief with latest streamed answers
