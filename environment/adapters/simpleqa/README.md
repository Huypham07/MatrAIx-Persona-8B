...
This adapter converts OpenAI SimpleQA into Matraix Playground-compatible task directories.
...
Generated task directories follow the Matraix Playground task layout:
...
Job outputs are written by Matraix Playground according to the recipe's `jobs_dir`. Do not
...
`claude-code@2.1.32` with `claude-opus-4-6` and an LLM judge. Detailed run
metadata was recorded during the parity run.
